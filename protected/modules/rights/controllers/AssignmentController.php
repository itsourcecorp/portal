...
{
        public $layout='//layouts/home_column1';
		...
		if( $assignSelectOptions!==array() )
		{
		    ...
		    if( isset($_POST['AssignmentForm'])===true )
			{
				...
				if( $formModel->validate()===true )
				{
                    ...
                    //assign tenant if applicable
                    $tenant = explode('.',$formModel->itemname);
                    if(isset($tenant[0]) AND is_numeric($tenant[0])){
                        $tenant = $tenant[0];
                        $tenant = Tenants::model()->findByPK($tenant);
                        if(!empty($tenant)){
                            $tenantassignment = new TenantAssignment();
                            $tenantassignment->user_id = $model->getId();
                            $tenantassignment->tenant_id=$tenant->id;
                            $oldassignment = TenantAssignment::model()->findByAttributes(array('user_id'=>$model->getId(),'tenant_id'=>$tenant->id));
	...
	*/
	public function actionRevoke()
	{
		...
		if( Yii::app()->request->isPostRequest===true )
		{
			...
			$item = $this->_authorizer->attachAuthItemBehavior($item);
            //remove tenant assignment from user if last role is gone
            $user = User::model()->findByPK($_GET['id']);
            $tenant = explode('.',$itemName);
            $tenant = $tenant[0];
            $tenant = Tenants::model()->findByPK($tenant);
            if(!empty($tenant)){
            $tenantassignment = TenantAssignment::model()->findByAttributes(array('user_id'=>$_GET['id'],'tenant_id'=>$tenant->id));
            $tenantroles = $user->getTenantRoles($tenant->id);
            if(empty($tenantroles) AND !empty($tenantassignment)){
